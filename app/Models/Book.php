...
final class Book
{
    ...
    public function latest(int $limit = 4): array
    {
        ...
        $statement = $this->db->prepare(
            'SELECT * FROM (
                SELECT b.id, b.title, b.subject, b.school_level AS level_label, b.condition_label, b.description,
    ...
    {
        $sql = 'SELECT b.id, b.title, b.subject, b.school_level AS level_label, b.condition_label, b.description,
                       b.owner_id, b.status, b.is_active, b.created_at, b.updated_at, u.name AS owner_name
                FROM books b
                INNER JOIN users u ON u.id = b.owner_id
                WHERE b.is_active = 1';
    ...
    public function create(array $data): int
    {
        $statement = $this->db->prepare(
            'INSERT INTO books (title, subject, school_level, condition_label, description, owner_id, status, is_active)
             VALUES (:title, :subject, :school_level, :book_condition, :description, :owner_id, :status, :is_active)'
        );
    ...
    {
        $statement = $this->db->prepare(
            'SELECT id, title, subject, school_level AS level_label, condition_label, description,
